...
class SuratKeluarController extends BaseController
{
    ...
    {
        $res = $this->m_surat->getSuratKeluar();
        $nomor = 1;
        $output = [];
    ...
    // // fungsi getData Verfikasi Surat Keluar
    ...
    {
        $res = $this->m_surat->getVerifikasiSuratKeluar(); // Ambil dari model
        $output = [];
        $nomor = 1;
        $level = session()->get('level');
    ...
    public function del_data()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = ['sukses' => 'Data surat keluar telah dihapus.'];
